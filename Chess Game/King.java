...
public class King extends ChessPiece {
    ...
    public King(char symbol, String currentPosition, int player) {
        super(6, symbol, currentPosition, player);
    }
    ...
    public void move(Player player, Player opponent, int currentRound, ChessPiece[][] board) {
        ...
        if (validifyPosition(desiredCords)) {
            if (!checkForPiece(board, desiredIndices) && verifyMovement(desiredCords) && !player.possibleCheck(opponent, desiredCords, board)) {
                ...
                System.out.println("\u001B[1m\u001B[32mKing moved from " + previousPosition + " to " + currentPosition + "\u001B[0m");
            ...
            if (capture(player, opponent, board, desiredCords) && !checkIfKing(desiredIndices, board)) {
                ...
                //Update board
                board[desiredIndices[0]][desiredIndices[1]] = this;
                board[currentIndices[0]][currentIndices[1]] = new ChessPiece(0, ' ', currentPosition, 0);
    ...
    public boolean capture(Player player, Player opponent, ChessPiece[][] board, String desiredCords) {
        ...
        if (validifyPosition(desiredCords) && checkIfPlayersPiece(opponent, desiredIndices, board)
           && verifyMovement(desiredCords) && !player.possibleCheck(opponent, desiredCords, board)) {
            return true;       
        }
        return false;
    }
    ...
    public boolean verifyMovement(String desiredCords) {
        if ((desiredCords.charAt(0) - currentPosition.charAt(0) == 1 || desiredCords.charAt(0) - currentPosition.charAt(0) == -1 || desiredCords.charAt(0) - currentPosition.charAt(0) == 0) 
           && (desiredCords.charAt(1) - currentPosition.charAt(1) == 1 || desiredCords.charAt(1) - currentPosition.charAt(1) == -1 || desiredCords.charAt(1) - currentPosition.charAt(1) == 0)) {
            return true;
        }
        return false;
    }
